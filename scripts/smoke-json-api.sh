...
"$picker" wait-agent "$pane" --json --timeout 2s --interval 1s | jq -e '.terminalStatus == "done" and .wakeDelivered == true' >/dev/null
...
monitor_json="$("$picker" monitor-agent "$pane" --json --timeout 30s --interval 5s)"
