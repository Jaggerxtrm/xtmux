...
tmux -L "$label" -f /dev/null new-session -d -s smoke 'sleep 60'
tmux -L "$label" split-window -d -t smoke: 'sleep 60'
socket="$(tmux -L "$label" display-message -p -t smoke: '#{socket_path}')"
pane="$(tmux -L "$label" list-panes -t smoke: -F '#{pane_id}' | head -1)"
target_pane="$(tmux -L "$label" list-panes -t smoke: -F '#{pane_id}' | tail -1)"
sid="$(tmux -L "$label" display-message -p -t "$pane" '#{session_id}')"
tmux -L "$label" set-option -p -t "$pane" @agent_state done
tmux -L "$label" set-option -p -t "$target_pane" @agent_state done
...
"$picker" wait-agent "$target_pane" --json --timeout 2s --interval 1s | jq -e '.terminalStatus == "done" and .wakeDelivered == true' >/dev/null
...
tmux -L "$label" set-option -p -t "$target_pane" @agent_state running
monitor_json="$("$picker" monitor-agent "$target_pane" --json --timeout 30s --interval 5s)"
