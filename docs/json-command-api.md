...
| Monitor | `monitorId`, `waitId` when linked, `target`, `requesterSessionId`, `requesterPaneId`, `sessionId`, `paneId`, `state`, `paneState`, `startedAtMs`, `updatedAtMs`, `timeoutMs`, `intervalMs`, `terminalStatus`, `terminalAtMs`, `wakeDelivered`, `wakeConsumed`, `orphan`. `state` reflects `terminalStatus` when the monitor is terminal; otherwise the observed pane state. `paneState` always carries the raw pre-overlay pane state so callers that need the split still have it (xtrm-wiy5n.4.16) |
| Monitor mutation | arm: `monitorId`, `waitId`, `target`, `requesterSessionId`, `requesterPaneId`, `sessionId`, `paneId`, `state`, `paneState`, `startedAtMs`, `timeoutMs`, `intervalMs`, `terminalStatus`, `wakeDelivered`; kill: `monitorId`, `status` |
...
| Outcome apply | `schemaVersion`, `status`, `reasonCode`, `paneId`, `sessionId`, `appliedState`, `duplicate`, `nextActions[]` (exact argv pass-through); consumes `xtrm.command-outcome.v1` from stdin and enforces the full published v1 schema boundary; K3 surface experimental until K5 (GATE-IFACE already passed; promotion/release follow K4/K5) |
...
| `picker:instance-get` | agent-json | `instance-get <instance-id> [--json]` → one `agent_instances` row; plain prints `field<TAB>value` lines for shell callers (`agent-state.sh` restart rehydration), `--json` the object; exit 5 `XTMUX_NOT_FOUND` if none. Requires `XTMUX_OBS_V2=1` | s96.4 |
...
| `picker:agent-episode` | agent-json | `agent-episode <pane-id\|session-id> [--json]` → the target's latest response episode (one user prompt + all continuations until control returns): `primary` + `followUps` substantive candidates, `collapsed` short acknowledgements; plain prints the primary text; exit 5 if none | gdk |
...
| `obs:agent-episode` | agent-json | latest `agent_episodes` row for a pane/session + its `agent_turns` candidates in order; conservative projection — first substantive candidate is `primary`, later substantive ones `followUps`, short hook acknowledgements `collapsed` (never primary). Rows attach via `episode_open=1` (fresh episode) / 0-or-absent (attach; lazy-open). Episodes close on the next `agent.episode.open` (Claude UserPromptSubmit hook) | gdk |
| `obs:instance-get` | agent-json | one `agent_instances` row by instance id. Default output is `field<TAB>value` lines so `agent-state.sh` can read it without a JSON parser; `--json` gives the object. Keyed on the INSTANCE id and never on the pane id: tmux recycles `%N`, so a pane-keyed lookup would hand a fresh agent the previous occupant's bead. Read-only | s96.4 |
| `obs:outcome-apply` | agent-json | consumes one `xtrm.command-outcome.v1` launch outcome from stdin and enforces the FULL published v1 schema boundary (every object closed, every field validated — a payload `@xtrm/contracts` would reject is never accepted). Structured JSON in/out, no prose parsing: unknown schema versions are refused (`XTMUX_UNSUPPORTED_SCHEMA`, rc 2), hook-trust bypass claims are refused (`XTMUX_HOOK_TRUST_VIOLATED`, rc 4), and hostile identities/nested keys/argv are refused (`XTMUX_INVALID_ARGUMENT`, rc 2). Only `status: degraded` with a `%N` pane writes — exactly one pane-scoped `degraded` transition; duplicate deliveries of the same identity are suppressed within one tmux server lifetime (not idempotent across a real restart: recycled `$N`/`%N` pairs can suppress a distinct later fact — K4/contract evolution); `nextActions` argv passes through verbatim. K3 surface: experimental until K5 (GATE-IFACE already passed; promotion/release follow K4/K5) | s96.2 |
