...
| Outcome apply | `schemaVersion`, `status`, `reasonCode`, `paneId`, `sessionId`, `appliedState`, `duplicate`, `nextActions[]` (exact argv pass-through); consumes `xtrm.command-outcome.v1` from stdin, experimental until K5 |
...
| `obs:outcome-apply` | agent-json | consumes one `xtrm.command-outcome.v1` launch outcome from stdin (K3, experimental until K5). Structured JSON in/out, no prose parsing: unknown schema versions are refused (`XTMUX_UNSUPPORTED_SCHEMA`, rc 2), hook-trust bypass claims are refused (`XTMUX_HOOK_TRUST_VIOLATED`, rc 4), and hostile identities/unknown keys/argv are refused (`XTMUX_INVALID_ARGUMENT`, rc 2). Only `status: degraded` with a `%N` pane writes — exactly one pane-scoped `degraded` transition, idempotent across replay and restart; `nextActions` argv passes through verbatim | s96.2 |
