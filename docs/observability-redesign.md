...
| `xtmux-auto-monitor` | Claude PostToolUse hook touches `<sanitized-target>_pending` after successful send/pointer recognition; it may touch even when another monitor is active. [`hooks/claude/auto-monitor-on-send.mjs:20-50`, `hooks/claude/auto-monitor-on-send.mjs:152-167`] | Claude Stop drain hook scans every `_pending` file; Pi does not read this directory. [`hooks/claude/auto-monitor-drain-stop.mjs:18-55`, `hooks/claude/auto-monitor-drain-stop.mjs:81-98`] | Claude PostToolUse consumed hook removes target marker after a wait-agent command; drain-stop prunes old markers; its reason also tells users to `rm -f` manually. [`hooks/claude/auto-monitor-consumed.mjs:10-12`, `hooks/claude/auto-monitor-consumed.mjs:36-45`, `hooks/claude/auto-monitor-drain-stop.mjs:41-51`, `hooks/claude/auto-monitor-drain-stop.mjs:73-76`] | `XTMUX_AUTO_MONITOR_TTL_MS`, default 3,600,000 ms; `XTMUX_AUTO_MONITOR_DRAIN_DISABLE=1` bypasses gate and `XTMUX_AUTO_MONITOR_DISABLE=1` bypasses sender hook. [`hooks/claude/auto-monitor-drain-stop.mjs:16-21`, `hooks/claude/auto-monitor-drain-stop.mjs:81-83`, `hooks/claude/auto-monitor-on-send.mjs:23-26`, `hooks/claude/auto-monitor-on-send.mjs:142-143`] | Touching same target updates mtime; it means another send observed, not a distinct requester/target wait. An existing monitor does not suppress touch. [`hooks/claude/auto-monitor-on-send.mjs:124-132`, `hooks/claude/auto-monitor-on-send.mjs:160-165`] | Old marker is pruned before Stop, so a forgotten monitor-arm gate silently stops blocking after TTL; foreign target identity cannot be distinguished from same-name target. [`hooks/claude/auto-monitor-drain-stop.mjs:31-55`] |
...
4. **Legacy target-only markers.** `xtmux-auto-monitor` stores only a sanitized
   target filename and no requester identity [`hooks/claude/auto-monitor-on-send.mjs:36-38`].
